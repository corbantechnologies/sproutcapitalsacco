...
import { bulkUpdateSavingsDepositsCSV, downloadBulkUpdateTemplate } from "@/services/savingsdeposits";
import { FileUp, X, Download, FileCheck } from "lucide-react";
import React, { useState, useRef } from "react";
import toast from "react-hot-toast";

function BulkSavingDepositEditUpload({ onBatchSuccess }) {
    ...
    const handleDownloadTemplate = async () => {
        try {
            await downloadBulkUpdateTemplate(token, depositType);
            toast.success("Prefilled template downloaded successfully!");
        } catch (error) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            await bulkUpdateSavingsDepositsCSV(formData, token);
            toast.success("Deposit dates updated successfully!");
            clearFile();
            if (onBatchSuccess) onBatchSuccess();
        } catch (error) {
            console.error("Upload error:", error.response?.data);
            toast.error(error?.response?.data?.error || "Failed to update deposits.");
        } finally {
            setLoading(false);
        }
    };
    ...
    return (
        <div className="max-w-3xl mx-auto space-y-8 py-4">
            <div className="text-center space-y-2">
                <h2 className="text-2xl font-bold text-emerald-800">Bulk Edit Deposit Dates (CSV)</h2>
                <p className="text-slate-500 text-sm max-w-lg mx-auto">
                    Download a prefilled template of existing deposits, edit the transaction dates, and upload to update them in bulk.
                </p>
            </div>
            ...
            <div className="bg-emerald-50 rounded p-6 border border-emerald-100 flex flex-col md:flex-row items-center justify-between gap-4 shadow-sm">
                ...
                <div className="flex gap-2">
                    <select
                        ...
                        className="border border-emerald-200 rounded px-3 py-1 text-sm bg-white"
                    >
...
export default BulkSavingDepositEditUpload;
